...
export const useBubbleAnimation = (config: BubbleAnimationConfig = {}) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      switch (type) {
        case "float":
          newState.y = Math.sin(t) * intensity * 8
          newState.x = Math.cos(t * 0.7) * intensity * 4
          break
        case "drift":
          newState.x = Math.sin(t) * intensity * 12
          newState.y = Math.cos(t * 0.5) * intensity * 6
          break
        case "pulse":
          newState.scale = 1 + Math.sin(t) * intensity * 0.1
          newState.y = Math.sin(t * 0.8) * intensity * 4
          break
        case "rotate":
          newState.rotate = t * 30
