...
import { ChevronDown, X, RefreshCw, Save, Maximize2 } from "lucide-react"
...
// -----------------------------
// Custom Coin Node – web-style: inner logo, dark ring, glowing green ring, label on hover
// -----------------------------
const CoinNode = React.memo<NodeProps>(function CoinNode({ data, id }) {
  ...
  return (
    <div className="relative flex flex-col items-center w-[80px] min-h-[100px]">
      ...
      />
      {/* TOKEN: circle + amber/gold ring so it’s clearly a token */}
...
// -----------------------------
// Custom Whale Node – web-style: inner grey + initial, green glow, label below; click = popup card
// -----------------------------
const WhaleNode = React.memo<NodeProps>(function WhaleNode({ data, selected, id }) {
  ...
  const displayName = (data.influencerName as string) || groupName || (address.length > 12 ? `${address.slice(0, 5)}...${address.slice(-6)}` : address || "Whale")
  const shortLabel = displayName.length > 14 ? `${displayName.slice(0, 10)}...` : displayName
  const initial = (data.influencerName as string)?.trim().slice(0, 1) || (address ? address.slice(0, 1) : "W")
  const buy = (data.totalBuyAmount as number) ?? 0
  const sell = (data.totalSellAmount as number) ?? 0
  const borderColor = buy >= sell ? "#06DF73" : "#df2a4e"
  ...
  return (
    <>
      {/* Whale popup – small, sharp, same fonty style */}
      <NodeToolbar
        isVisible={!!selected}
      ...
      >
        <div
          className="p-2 font-sans antialiased text-[11px] rounded-none"
          style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}
        >
          <div className="flex items-center justify-between gap-1 mb-1.5">
            <span className="text-[9px] font-semibold uppercase tracking-wider text-[#666]">Address</span>
            <button
              onClick={(e) => {
                e.stopPropagation()
                handleClose()
              }}
              className="text-[#555] hover:text-white p-0.5"
              aria-label="Close"
            >
              <X className="w-3 h-3" />
            </button>
          ...
          <div className="flex items-center gap-1 mb-2">
            ...
            <div className="flex items-center gap-0.5 shrink-0">
              <button
                ...
                className="p-1 rounded text-[#666] hover:text-[#06DF73]"
                aria-label="Copy"
              >
      ...
      <div className="relative flex flex-col items-center cursor-pointer w-[70px] min-h-[90px]">
        ...
        />
        {/* WHALE: rounded square + blue ring so it’s clearly a wallet/person */}
        <div className="relative w-[72px] h-[72px] flex items-center justify-center shrink-0">
          <div
            className="relative w-16 h-16 rounded-full bg-black flex items-center justify-center p-[3px] shrink-0"
            style={{
          ...
          >
            <div
              className="w-full h-full rounded-full flex items-center justify-center p-[3px] shrink-0 bg-[#484848]"
              style={{ borderColor: "#484848", borderWidth: 2, borderStyle: "solid" }}
            >
              <div className="w-full h-full rounded-full bg-black flex items-center justify-center p-[2px] shrink-0 overflow-hidden">
...
// -----------------------------
// Custom Edge – one edge per (token, whale), draws buy and/or sell line(s)
// -----------------------------
const CustomEdge = React.memo<EdgeProps>(function CustomEdge({
  id,
  sourceX,
  sourceY,
  targetX,
  ...
  if (isMixed) {
    if (hasBuy) {
      ...
      paths.push({ d: `M ${startX} ${startY} L ${endX} ${endY}`, stroke: "#06DF73", strokeWidth: isHovered ? 1.25 : 1 })
    }
    if (hasSell) {
      const spread = step * spreadMult
      const startX = sourceX + nx * spread
      ...
      const endX = targetX - nx * spread
      const endY = targetY - ny * spread
      paths.push({ d: `M ${startX} ${startY} L ${endX} ${endY}`, stroke: "#df2a4e", strokeWidth: isHovered ? 2 : 1.5 })
    }
  } else {
...
const DownloadButton: React.FC<{
  ...
  filename?: string
}> = ({ onDownloadReady, onScreenshotError, filename = "whale-network-graph" }) => {
...
const WhaleNetworkGraph: React.FC<{
  ...
  const fetchData = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        // const whaleCount = customWhales || filters.whales (Unused since we force minWhales=1)
        ...
        // Use correct endpoint for whales
        // FIXED: Always fetch with minWhales=1 to allow client-side limiting (User wants "Top N" not "Min N")
        const res = await axios.get(
          `${BASE_URL}/whale/visualize-whales?timeframe=${filters.timeframe}&minWhales=1&minInflow=${numericVolume}`
        )
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    // const now = Date.now() (Unused since redundant filtering removed)
    const filteredTrades = allTrades // Backend handles filtering
    // Removed redundant client-side time check to avoid clock skew issues

    // Client-side Limit Logic: Filter for Top N Whales Global by Volume
    const limitCount = parseInt(filters.whales, 10) || 100

    // 1. Group trades by Whale Address to calculate total volume
    const whaleVolumes: Record<string, number> = {}
    allTrades.forEach((trade) => {
      const vol = Number(trade.amount) || 0
      whaleVolumes[trade.whaleAddress] =
        (whaleVolumes[trade.whaleAddress] || 0) + vol
    })

    // 2. Sort whales by volume desc and take top N
    ...
    // 3. Filter trades to only include these top whales
    ...
    // Global whale lookup from API (any coin) for image/profile
    ...
    Object.values(tradesByCoin).forEach((coinData, coinIndex) => {
      ...
      const tradesByWhale = coinData.trades.reduce(
        (
          acc: Record<
            string,
            { whaleAddress: string; trades: any[]; influencerName?: string; influencerProfileImageUrl?: string; whaleLabel?: string[] }
          >,
          trade: any
        ) => {
      ...
      Object.entries(tradesByWhale).forEach(([whaleId, whaleData], whaleIndex) => {
        ...
        flowNodes.push({
          ...
          data: {
            ...
            whaleLabel: firstTrade?.whaleLabel ?? whaleData.whaleLabel ?? apiWhale?.whaleLabel,
          },
        })
  ...
  // Fit view when we have nodes and (panel just opened or instance ready)
  ...
  const handleActivateWhaleClusterAlert = useCallback(() => {
    const token = localStorage.getItem("accessToken")
    if (!token) {
      showToast("Please log in to activate Whale Cluster alerts", "error")
      return
    }
    ...
    validateAccess(async () => {
      ...
      try {
        ...
        const response = await api.post("/alerts/whale-cluster", {
          timeWindowMinutes,
          minClusterSize,
        ...
        if (response.data?.success) {
          setIsSaved(true)
          showToast("Whale Cluster alert activated", "success")
        } else {
          showToast(
        ...
        const msg =
          ...
          err.message ||
          "Failed to activate Whale Cluster alert"
        showToast(msg, "error")
      } finally {
        ...
        <div className="absolute inset-0 z-0 w-full h-full bg-black">
          ...
          {loading ? (
            <div className="flex items-center justify-center h-full">
              <div className="animate-spin rounded-none h-12 w-12 border-2 border-[#333] border-t-transparent" />
            </div>
          ) : nodesState.length === 0 ? (
            <div
        ...
        <div className="relative z-10 pointer-events-none w-full h-full flex flex-col justify-start p-3 md:p-6 lg:p-8 gap-4">
          ...
          <div className="w-full pointer-events-auto">
                ...
                >
                  {!isSaved && (
                    <div className="parent-dropdown-content">
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              if (!user?.telegramChatId) {
                                ...
                                handleActivateWhaleClusterAlert()
                              }
                            }}
                          >
                      ...
                      <div className="custom-frm-bx position-relative">
                        <label className="nw-label">Min whale wallets</label>
                        <div
                          className="form-select cursor-pointer text-start"
                          onClick={(e) => {
                            e.stopPropagation()
            ...
            >
              ...
              {dropdown === "whales" && (
                <div className="filter-dropdown-menu w-full">
                  <div className="filter-dropdown-header">Whales</div>
                  {whaleOptions.map((opt) => (
                    <button
                      key={opt}
...
export default WhaleNetworkGraph
