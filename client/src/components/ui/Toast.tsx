...
export interface ToastProps {
  ...
  variant?: "standard" | "processing" | "transaction"
  txSignature?: string
  duration?: number
    ...
    <div className="coppied-address">
      <div className="coppied-content">
        <span className={`coppied-icon ${type === 'error' ? 'text-red-500' : ''}`}>
          {type === 'error' ? <XIcon size={16} /> : <FaRegCheckSquare />}
        </span>
        <p>{message}</p>
        <button
          onClick={() => onClose(id)}
...
interface ToastContextType {
  showToast: (message: string, type?: ToastProps["type"], variant?: ToastProps["variant"], options?: { duration?: number, txSignature?: string }) => string | null
...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const showToast = useCallback((
    ...
    options?: { duration?: number, txSignature?: string }
  ) => {
