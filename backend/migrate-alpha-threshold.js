...
const { validateSOLBalance, PREMIUM_BALANCE_THRESHOLD } = require('./dist/middlewares/premiumGate.middleware')
  ...
  try {
    ...
    for (const user of usersWithTelegram) {
      try {
        ...
        // Check ALPHA balance
        const balanceResult = await validateSOLBalance(walletForCheck, true)
        ...
        if (!balanceResult.hasAccess) {
          ...
          try {
            ...
            console.log(`   ⚠️  Failed to send Telegram message: ${telegramError.message}`)
          }
