...
/**
 * POST /api/v1/alerts/whale-cluster
 * Create or update Whale Cluster alert (same flow as whale-alert: Telegram + premium required)
 */
export const createWhaleClusterAlert = catchAsyncErrors(
  async (req: Request, res: Response) => {
    const userId = (req as any).userId
    ...
    if (clusterSize < 1 || clusterSize > 50) {
      return res.status(400).json({
        success: false,
        message: 'Min whale wallets must be between 1 and 50',
      })
    }
    ...
    try {
      ...
      const existing = await UserAlert.findOne({ userId, type: AlertType.WHALE_CLUSTER })
      const isUpdate = !!existing

      const alert = await UserAlert.findOneAndUpdate(
        { userId, type: AlertType.WHALE_CLUSTER },
        {
      ...
      telegramService.sendAlertConfirmation(
        ...
        logger.warn({
          component: 'AlertController',
          operation: 'createWhaleClusterAlert',
          userId,
      ...
      logger.info({
        ...
        alertId: alert._id,
        config,
        message: 'Whale Cluster alert subscription created/updated successfully',
      })
      ...
      res.status(500).json({
        success: false,
        message: 'Failed to create Whale Cluster alert subscription',
      })
    }
  },
)
