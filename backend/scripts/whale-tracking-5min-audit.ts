/**
 * Whale Tracking 5-Minute Audit
 *
 * 1. Load ALL whale addresses from MongoDB (whalesaddresses collection)
 * 2. Subscribe to Helius WebSocket with those addresses (using env HELIUS_API_KEY / WSS_URL)
 * 3. For 5 minutes: accept every transaction notification (no whale-address matching;
 *    we're only subscribed to our whales, so every tx is from our list)
 * 4. Parse each tx via Shyft getParsedTransactions + V2 parser; keep only SWAP txns
 * 5. Store each txn with wallet-as-is (parser's swapper); no filtering by tracked list
 * 6. Write all txns to CSV in docs/
 * 7. Write report: unique wallets in txns, how many of those are in tracked whales
 *
 * Run: npx ts-node scripts/whale-tracking-5min-audit.ts (from backend/)
 */
...
const PARSE_QUEUE_DRAIN_MS = 800 // delay between batch parses to avoid Shyft rate limit
...
// Output paths (project root docs/)
const DOCS_DIR = path.join(__dirname, '../../docs')
const CSV_PATH = path.join(DOCS_DIR, 'whale_5min_txns.csv')
const REPORT_PATH = path.join(DOCS_DIR, 'WHALE_5MIN_AUDIT_REPORT.md')
...
function writeReport(trackedWhales: string[], dbComparison: DbComparison | null): void {
  const trackedSet = new Set(trackedWhales.map((a) => a.trim()).filter(Boolean))
...
| Metric | Audit (this run) | DB (whaleAllTransactionsV2) |
|--------|------------------|----------------------------|
...
`
  ...
  const report = `# Whale Tracking 5-Minute Audit Report

**Run:** ${new Date().toISOString()}  
**Duration:** 5 minutes  
**Helius:** Subscribed to all whale addresses from DB (no address matching on receive).

## Summary

| Metric | Value |
|--------|--------|
| Tracked whales (from DB) | ${trackedWhales.length} |
...
- **CSV:** \`docs/whale_5min_txns.csv\` (signature, wallet, type, tokenIn, tokenOut, amountIn, amountOut, timestamp, confidence)
- **Report:** \`docs/WHALE_5MIN_AUDIT_REPORT.md\` (this file)

## Notes

- No whale-address matching was applied on WebSocket receive: every \`transactionNotification\` from Helius was accepted (we subscribed only to our addresses).
- Wallet stored as-is from the parser (swapper). No filter requiring swapper to be in tracked list.
...
async function main(): Promise<void> {
  console.log('Loading whale addresses from MongoDB...')
  const whaleAddresses = await getWhaleAddresses()
  console.log(`Loaded ${whaleAddresses.length} whale addresses`)

  if (whaleAddresses.length === 0) {
    console.error('No whale addresses found. Exiting.')
    process.exit(1)
  }
  ...
  ws.on('open', () => {
    console.log('Helius WebSocket connected. Subscribing to all whale addresses...')
    const request = {
      jsonrpc: '2.0',
      id: 1,
  ...
  // Drain queue every 500ms while running
  ...
  writeReport(whaleAddresses, dbComparison)
  console.log('Done.')
  process.exit(0)
}
