/**
 * Migration Script: Fix Influencer Split Swap Transactions
 * 
 ...
 * - Only updates transactions from last 20 hours
 * - Validates each transaction before updating
 * - Creates backup of affected records
 * - Detailed logging of all changes
 * 
 * USAGE:
 *   node migrate-influencer-split-swaps.js              # Dry run (preview only)
 *   node migrate-influencer-split-swaps.js --apply      # Apply changes
 */
...
}, { collection: 'influencerwhaletransactionsv2', strict: false });

const Transaction = mongoose.model('InfluencerTransaction', transactionSchema);
...
async function migrateSplitSwaps() {
  try {
    console.log('🔧 Influencer Split Swap Migration Script');
    ...
    if (DRY_RUN) {
      ...
      console.log('   To apply changes, run: node migrate-influencer-split-swaps.js --apply\n');
    } else {
    ...
    if (DRY_RUN) {
      ...
      console.log(`   node migrate-influencer-split-swaps.js --apply`);
      return;
    }
    ...
    const backupFile = path.join(backupDir, `influencer-split-swap-backup-${Date.now()}.json`);
