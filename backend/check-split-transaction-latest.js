...
const WhaleAllTransactionsV2 = mongoose.model('whaleAllTransactionV2', whaleAllTransactionsV2Schema);

async function checkSplitTransaction() {
  try {
    ...
    const transactions = await WhaleAllTransactionsV2.find({ signature }).lean();
    
    console.log(`📊 RESULT: Found ${transactions.length} record(s) with this signature\n`);
    
    if (transactions.length === 0) {
      console.log('❌ No transactions found with this signature');
      ...
      console.log(`   Labels: ${txn.whale?.labels?.join(', ') || 'N/A'}`);
