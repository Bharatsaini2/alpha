 ...
 * 4. Creates automatic backup before migration
 */
...
async function migrateLast24HoursSplitTransactions() {
  ...
  try {
    ...
    for (const { name: collectionName, type } of collections) {
      ...
      // Find old "both" type transactions with non-core tokens FROM LAST 24 HOURS
      const query = {
        type: 'both',
      ...
      console.log(`   Time filter: ${TWENTY_FOUR_HOURS_AGO.toISOString()} to now\n`);
      ...
      for (const oldTx of oldTransactions) {
        try {
          ...
          // Extract amounts from bothType array or direct fields
          const bothType = oldTx.bothType?.[0] || {};
          const sellAmount = oldTx.tokenAmount?.sellTokenAmount || oldTx.amount?.sellAmount || 0;
          const buyAmount = oldTx.tokenAmount?.buyTokenAmount || oldTx.amount?.buyAmount || 0;
          const sellValue = oldTx.amount?.sellAmount || 0;
          const buyValue = oldTx.amount?.buyAmount || 0;
          ...
          const sellRecord = {
            ...
            type: 'sell',
            classification: 'migrated_split_sell_24hrs',
          ...
          const buyRecord = {
            ...
            _id: undefined, // Let MongoDB generate new ID
            type: 'buy',
            classification: 'migrated_split_buy_24hrs',
            originalId: oldTx._id,
            originalType: 'both',
            migratedAt: new Date(),
            isBuy: true,
          ...
          if (DRY_RUN) {
            ...
            console.log(`     - SELL: ${oldTx.tokenInSymbol} (${sellAmount} tokens, $${sellValue.toFixed(2)})`);
            console.log(`     - BUY: ${oldTx.tokenOutSymbol} (${buyAmount} tokens, $${buyValue.toFixed(2)})`);
          } else {
            // Insert new records
            ...
            await collection.updateOne(
              ...
              { 
                $set: { 
                  migrated: true,
                  migratedAt: new Date(),
                  migratedTo: 'split_transactions_24hrs'
                }
              }
            );
      ...
      console.log('\n' + '─'.repeat(80));
      console.log(`\n📊 Migration Summary for ${type} (Last 24 Hours):`);
...
console.log(`Time filter: Last 24 hours (since ${TWENTY_FOUR_HOURS_AGO.toISOString()})\n`);

migrateLast24HoursSplitTransactions();
